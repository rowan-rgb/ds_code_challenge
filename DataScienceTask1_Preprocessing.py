...
print(data_1_hex.columns)
data_1_hex.to_csv("data_1_hex.csv", index=False)

#Thus ends our data visualisation and attribution. 

#Data preprocessing for our predictive model.
# Load data
data_1_hex = pd.read_csv(
    "https://cct-ds-code-challenge-input-data.s3.af-south-1.amazonaws.com/sr_hex.csv.gz"
)

# Rename columns if needed
data_1_hex.columns = [
    "hex_index" if col == "h3_level8_index" else col
    for col in data_1_hex.columns
]

# Convert timestamps
data_1_hex["creation_timestamp"] = pd.to_datetime(data_1_hex["creation_timestamp"])

# Create week start column
data_1_hex["week_start"] = data_1_hex["creation_timestamp"].dt.to_period("W").apply(lambda r: r.start_time)

# Group by week and hex
weekly_counts = (
    data_1_hex
    .groupby(["week_start", "hex_index"])
    .size()
    .reset_index(name="count")
)

# Remove invalid hexes
weekly_counts_new = weekly_counts[weekly_counts["hex_index"] != "0"]

# Pivot counts
pivoted = weekly_counts_new.pivot(
    index="hex_index",
    columns="week_start",
    values="count"
)
...
pivoted = (
    pivoted.apply(pd.to_numeric, errors="coerce")
    .fillna(0)
    .sort_index(axis=1)
)
pivoted.columns = list(range(1, pivoted.shape[1] + 1))
pivoted.to_csv("pivoted_weekly_counts.csv")
